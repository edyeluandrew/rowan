...
export default function ReviewModal({ transactionId, traderName, onClose, onSubmitted }) {
  ...
  const handleSubmit = async () => {
    ...
    setError(null)
    try {
      await submitReview({ transactionId, rating, comment: comment.trim() || undefined })
      setSuccess(true)
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center p-4">
      <div className="bg-rowan-surface border border-rowan-border rounded-2xl w-full max-w-md p-5">
        ...
        <p className="text-rowan-muted text-sm mb-5">
          Rate your experience with {getTraderDisplayName(traderName)}
        </p>
