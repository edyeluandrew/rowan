...
export async function check2faStatus() {
  const { data } = await client.get('/api/v1/trader/auth/2fa/status');
  return data;
}
...
export async function initiate2faSetup() {
  const { data } = await client.post('/api/v1/trader/auth/2fa/setup');
  return data;
}
...
export async function verifyTwoFactorSetup(code) {
  const { data } = await client.post('/api/v1/trader/auth/2fa/verify-setup', { code });
  return data;
}
...
export async function verifyTwoFactorLogin(traderId, code) {
  ...
  const response = await fetch(
    `${import.meta.env.VITE_API_URL}/api/v1/trader/auth/2fa/verify-login`,
    {
      method: 'POST',
...
export async function disableTwoFactor(code) {
  const { data } = await client.post('/api/v1/trader/auth/2fa/disable', { code });
  return data;
}
...
export async function regenerateBackupCodes(code) {
  const { data } = await client.post('/api/v1/trader/auth/2fa/backup-codes/regenerate', { code });
  return data;
}
