  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-rowan-surface rounded-xl border border-rowan-border p-4">
                ...
                <div className="flex gap-2">
                  <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Add a note..." rows={2} className="flex-1 bg-rowan-bg border border-rowan-border text-rowan-text rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-rowan-yellow resize-none" />
                  <button onClick={handleAddNote} disabled={!note.trim() || noteLoading} className="text-rowan-yellow hover:text-rowan-text disabled:opacity-50 self-end p-2">
                    <Send size={16} />
                  </button>
                </div>
              </div>
        ...
        <ConfirmDialog open={escalateConfirm} onClose={() => setEscalateConfirm(false)} onConfirm={handleEscalate} title="Escalate Dispute" message={`This will escalate dispute ${id} to a higher priority and cannot be undone. Are you sure?`} confirmLabel="Escalate" loading={actionLoading} variant="danger" />
      </div>
    </>
  )
}
