...
import { getTrader, approveTrader, suspendTrader, reactivateTrader, updateTraderLimits, adjustFloat } from '../../../shared/services/api/traders'
import { logAdminAction } from '../../../shared/services/api/system'
import { formatCurrency, formatNumber, formatPercent, formatDateTime, formatAddress } from '../../../shared/utils/format'
import { SUSPEND_REASON_MIN_LENGTH } from '../../../shared/utils/constants'
...
export default function TraderDetailPage() {
  ...
  const fetchTrader = useCallback(async () => {
    ...
    try {
      ...
      setTrader(t)
      setLimitForm({ daily_limit: String(t.daily_limit_ugx || t.daily_limit || ''), monthly_limit: String(t.monthly_limit || '') })
    } catch (err) {
  ...
  const TABS = [{ value: 'overview', label: 'Overview' }, { value: 'settings', label: 'Settings' }]
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {loading && !trader ? (
          ...
          <>
            <div className="bg-rowan-surface rounded-xl border border-rowan-border p-4 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                ...
                {trader.status === 'ACTIVE' && (
                  <Button variant="danger" onClick={() => { setSuspendError(''); handleAction('suspend', 'Suspend Trader', `Suspend ${trader.name || trader.id}? They will not receive new transactions.`) }}>
                    Suspend
                  </Button>
                )}
            ...
            <div className="flex gap-1 bg-rowan-surface rounded-xl p-1 border border-rowan-border w-fit">
              {TABS.map((t) => (
                <button key={t.value} onClick={() => setTab(t.value)} className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${tab === t.value ? 'bg-rowan-yellow/10 text-rowan-yellow font-medium' : 'text-rowan-muted hover:text-rowan-text'}`}>
                  {t.label}
                </button>
              ))}
            </div>
            ...
            {tab === 'settings' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-rowan-surface rounded-xl border border-rowan-border p-4 space-y-3">
                  ...
                  <Input label="Daily Limit (USD)" type="number" value={limitForm.daily_limit} onChange={(e) => { setLimitForm({ ...limitForm, daily_limit: e.target.value }); setLimitErrors({}) }} />
                  {limitErrors.daily_limit && <p className="text-rowan-red text-xs">{limitErrors.daily_limit}</p>}
                  <Input label="Monthly Limit (USD)" type="number" value={limitForm.monthly_limit} onChange={(e) => { setLimitForm({ ...limitForm, monthly_limit: e.target.value }); setLimitErrors({}) }} />
                ...
                <div className="bg-rowan-surface rounded-xl border border-rowan-border p-4 space-y-3">
                  ...
                  <Input label="Amount (USD)" type="number" value={floatAmount} onChange={(e) => { setFloatAmount(e.target.value); setFloatErrors({}) }} placeholder="Amount to add or subtract" />
                  ...
                  <div>
                    ...
                    <textarea value={floatNote} onChange={(e) => { setFloatNote(e.target.value); setFloatErrors({}) }} placeholder="Reason for adjustment..." rows={2} className="w-full bg-rowan-bg border border-rowan-border text-rowan-text rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-rowan-yellow resize-none" />
                  </div>
        ...
        <ConfirmDialog open={confirm.open} onClose={() => { setConfirm({ open: false, action: null, title: '', message: '' }); setSuspendReason(''); setSuspendError('') }} onConfirm={() => { if (confirm.action === 'suspend' && suspendReason.trim().length < SUSPEND_REASON_MIN_LENGTH) { setSuspendError(`Reason must be at least ${SUSPEND_REASON_MIN_LENGTH} characters`); return }; executeAction() }} title={confirm.title} message={confirm.message} loading={actionLoading} variant={confirm.action === 'suspend' ? 'danger' : 'primary'} confirmLabel={confirm.action === 'approve' ? 'Approve' : confirm.action === 'suspend' ? 'Suspend' : confirm.action === 'reactivate' ? 'Reactivate' : 'Confirm'}>
          {confirm.action === 'suspend' && (
            <div className="mt-3">
              <label className="text-rowan-muted text-xs block mb-1">Suspension Reason</label>
              <textarea value={suspendReason} onChange={(e) => { setSuspendReason(e.target.value); setSuspendError('') }} placeholder="Reason for suspension (min 10 chars)..." rows={2} className="w-full bg-rowan-bg border border-rowan-border text-rowan-text rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-rowan-yellow resize-none" />
